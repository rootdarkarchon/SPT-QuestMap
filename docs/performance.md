...
- The exact-version client starts one non-blocking topology warm-up after its one-shot plugin coroutine observes `CurrentScreenSingletonClass` reporting the stable main-menu screen and yields one additional frame. QuestMap does not patch the shared `MainMenuControllerClass.ShowScreen` path. The warm-up uses the same adapter, normalized topology, deterministic layout, and in-flight task as the Tasks-screen path; a screen opened during warm-up waits on that request instead of starting another. A background-only failure leaves vanilla UI untouched and permits the ordinary screen-triggered retry. This moves the measured request/deserialization/layout work out of the first Tasks-screen transition while leaving the live EFT overlay and screen mount on the main thread when the screen actually opens. `QUESTMAP_M02_PRELOAD` distinguishes start, completion, a waiting screen, and retryable failure.
- `QUESTMAP_M02_TOPOLOGY` and `QUESTMAP_M02_OVERLAY` retain topology, overlay, and layout timings. Topology telemetry separates server request, response bytes, JSON deserialize, topology normalize, and profile-projection time so a server-side data regression is not misattributed to layout. Server `QUESTMAP_M08_SERVER_PRELOAD` records the one-time item/location and loose-loot-derived quest-item map snapshot at the beginning of the post-database phase. `QUESTMAP_M08_SERVER_TOPOLOGY_BUILD`, `QUESTMAP_M08_SERVER_PROFILE_BUILD`, and `QUESTMAP_M08_SERVER_TOPOLOGY_ROUTE` further divide a cold request into cached data snapshots; node/objective/map construction and its slowest quest; seasonal and effective-requirement propagation; finalization; profile availability/applicability/state/frontier/repeatable construction; feed projection; serialization; and response character count. `QUESTMAP_M05_RENDER` adds first-render/node/edge/active-pool counts. Debug mode adds edge-mesh, overlay-refresh, and disposal high-water metrics without per-frame logging.

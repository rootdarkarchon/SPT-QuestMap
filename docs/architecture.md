...
- `QuestZoneMapCatalog` loads the shipped internal-map-to-trigger catalog once; the server resolves every objective's trigger IDs, classifies its raid relevance and exact location scopes, and computes each quest's authoritative concrete display-map union before either web or native-client transport;
...
Zone-derived map data remains distinct from Tarkov's native quest location. Each objective carries its original zone IDs, server-resolved internal map IDs, unresolved zone IDs, explicit in-raid relevance, and localized/image-ready `TaskLocations`. Known menu-only condition types are classified out of raid; every other type, including an unknown modded or future type, defaults to in-raid relevance. An out-of-raid objective receives only `Out of Raid`. An in-raid objective uses resolved concrete maps when available, otherwise its quest location as a fallback; native Any and Transition become explicit scopes, and Transition is retained alongside any concrete task maps. The quest-level `ActualMaps` union is computed from these task scopes for every quest. Header presentation uses concrete maps over Any over Out of Raid, while Transition remains first on transit quests. Factory day/night and Ground Zero low/high share the canonical internal map IDs supplied by `Data/triggerIds.json`. The Arena-only `develop` location is excluded at the server task-map catalog boundary, including mixed explicit conditions such as Safe Corridor's Reserve/Arena location list. Native table membership and per-row objective visibility intersect selected filters with exact task scopes; `Any` is not a wildcard. The server also transports canonical aliases for each applicable loaded location's internal `Id` and Mongo `_Id`, so raid matching needs no client-side location table or Factory/Ground Zero special case.

...
Objective density is bounded: the table shows four matching tasks per quest by default and provides an explicit row expander for the remainder. A persisted filter hides completed tasks. Completed objectives use a checkmark and omit both numerical `current / required` text and redundant full progress bars. Ready-to-turn-in quests report 100% overall progress. Quest and location images are width-driven at their original aspect ratio and vertically centered, so expanding a row never stretches its artwork to the row height.
...
QuestMap also owns in-raid quest-progression notifications as a product feature. Notifications are limited to effectively tracked quests. Manual tracking is profile-scoped in QuestMap's private `BepInEx/config/SPTQuestMap/tracking-state.json` document and is not represented by an F12 entry. F12 exposes live policies for automatically tracking newly accepted quests, implicitly tracking native favorites, and implicitly tracking quests assigned to the current raid map. Current-map tracking does not include `Any`, transit, or marathon quests. Removing manual tracking cannot override an enabled favorite/current-map policy. Reuse compact fading top-right cards grouped by trader/quest; a newer update replaces its quest/task/count/progress content and resets the visible duration. When aggregate and child objectives mutate together, show the most-specific changed leaf task. Paint numerical progress on the bar and omit redundant status text. The configurable notification opacity applies only to the normal card's composited quest-art/backdrop layers and the translucent black backgrounds of minimal notifications and the tracked list; it fades each complete background surface once against the raid view rather than blending translucent artwork into an opaque black card. Text, portraits, rails, and progress bars stay opaque outside the fade animation. A shared configurable fade duration controls both fade directions, while the notification and tracked-list fully-visible durations are separate settings. A default-off minimal-notification setting reduces each card to a small translucent text surface containing quest, bracketed progress, and objective only. Preserve quest and objective IDs at the policy boundary.

The configurable tracked-list hotkey (default `L`) toggles a compact fading panel at right-center during a raid. It automatically fades out after its separately configured display duration. It includes only effectively tracked active quests relevant to the current map, Any, or transit; groups them in the established natural trader order; sorts quests by localized name inside each group; and lists only unfinished objectives. Each trader group owns a portrait-only left column and vertical divider spanning that trader's quests; quest and objective content is rendered to the right. Non-binary objective progress is painted as text on its progress bar. Construct this projection only when the panel opens or when tracking/quest state changes while it is visible; it must not add topology rebuilds or complete-list work to the 100 ms raid monitor.

The trader header must use a single Unity `Image` for its fallback surface and asynchronously loaded portrait. Attempting to attach a second UI graphic aborts construction after only the trader initials appear. Each explicit list rebuild logs group, quest, objective, and content-height counts; a renderer exception produces both a visible failure row and the full exception in the client log.
...
- default and minimal notification presentation, including live F12 setting changes on the next notification;
- tracked-list hotkey toggle, timed automatic fade-out, portrait-column grouping with per-trader vertical dividers, quest-name ordering, current-map/Any/transit scope, open-objective filtering, and numeric progress bars;
...
Location selection is intentionally session/raid-local rather than persisted. A new game process initializes every location represented by the current Tasks projection; entering a raid resets to Any, Transit, and the current map; leaving the raid initializes the out-of-raid set again. When an overlay transition makes a quest newly Available, In Progress, Ready to Finish, or restartable, that quest's location is enabled automatically so a stale manual filter cannot silently hide the new row.
...
- Confirm fresh-process and raid-boundary location initialization, including automatic enabling when a newly available or accepted quest introduces a previously disabled map.
...
Post-closure extension (2026-08-09): In Progress now has a persisted `↻ ACTIVE / ↻ ALL` repeatable filter. All adds available Daily/Weekly quests to the active/ready/restartable set without adding ordinary available quests. Daily/Weekly banners expose native replacement only when EFT reports `IsChangeAllowed`; task rows expose native handover only when the exact-build `QuestObjectiveView` reports its stock handover control active and interactable. All quest-mutation controls are absent and callback-guarded in raid. Compact Collector/Lightkeeper strips reuse the graph palette on the right side of each quest banner.

Post-closure correction (2026-08-09): live objective snapshots now apply each normalized objective's comparator to its native checker value. This covers child/aggregate conditions where EFT omits a satisfied child ID from `CompletedConditions` even though its checker reached the requirement. Binary completion therefore remains consistent across notifications, Tasks rows, details, and overall progress. Normal raid notifications omit the redundant `1/1` rail, and raid-only Tasks/tracked-list projections retain only `Started` quests rather than showing hand-in-ready rows.

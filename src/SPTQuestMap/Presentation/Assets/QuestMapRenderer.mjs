...
function displayMapReferences(node){const scope=node.taskLocation?.id;if((scope==='any'||scope==='transition')&&node.actualMapsComplete&&node.actualMaps?.length){if(scope==='transition'&&node.actualMaps.length>1)return[node.taskLocation,...node.actualMaps];return node.actualMaps;}return node.taskLocation?[node.taskLocation]:[];}

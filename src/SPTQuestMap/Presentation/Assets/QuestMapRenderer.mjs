...
function displayMapReferences(node){const scope=node.taskLocation?.id;if(node.actualMaps?.length){if(scope==='transition')return[node.taskLocation,...node.actualMaps];return node.actualMaps;}return node.taskLocation?[node.taskLocation]:[];}

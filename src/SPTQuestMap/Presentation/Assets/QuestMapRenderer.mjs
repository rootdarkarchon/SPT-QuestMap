...
const STATE_THEME_PROPERTIES = {
  ...
  Expired:'--qm-state-expired', Pending:'--qm-state-pending', NotApplicable:'--qm-state-not-applicable'
};
const RENDER_THEME_PROPERTIES = {
  ...
  badgeScav:'--qm-renderer-badge-scav', badgeScavText:'--qm-renderer-badge-scav-text',
  ...
  compareDifference:'--qm-renderer-compare-difference', compareDifferenceText:'--qm-renderer-compare-difference-text'
};

const state = {
  canvas:null, viewport:null, metrics:null, dotnet:null, ctx:null, theme:null, resizeObserver:null, listeners:[],
  topology:null, profile:null, comparisonProfile:null, compareMode:false, staticNodeById:new Map(), nodeById:new Map(), stateById:new Map(), comparisonStateById:new Map(), incoming:new Map(), outgoing:new Map(),
  repeatableGroups:[], repeatableEndById:new Map(), serverTime:0, serverTimeStarted:0, clockTimer:0,
  normalLayout:null, activeLayout:null, applicable:new Set(), primaryApplicable:new Set(), comparisonApplicable:new Set(), visible:new Set(), comparisonById:new Map(), collectorPath:new Set(), lightkeeperPath:new Set(),
...
export async function refresh(snapshot,initial=false){
  try{
    const topology=snapshot?.topology,profile=snapshot?.profile??null,comparisonProfile=snapshot?.comparisonProfile??null,config=snapshot?.view;
    if(!topology||!config)throw new Error('QuestMap received an incomplete graph snapshot.');
    const previousVersion=state.topology?.version,previousProfile=`${state.profile?.profileId||''}|${state.comparisonProfile?.profileId||''}`;
    applyTopology(topology);applyProfiles(profile,comparisonProfile);applyConfig(config);
    ...
    const profileChanged=previousProfile!==`${profile?.profileId||''}|${comparisonProfile?.profileId||''}`;
...
function applyProfiles(profile,comparisonProfile){
  state.profile=profile;state.comparisonProfile=comparisonProfile;state.compareMode=Boolean(comparisonProfile);state.nodeById=new Map(state.staticNodeById);state.stateById=new Map((profile?.quests||[]).map(item=>[item.questId,item]));state.comparisonStateById=new Map((comparisonProfile?.quests||[]).map(item=>[item.questId,item]));
  state.primaryApplicable=new Set(profile?.allApplicableQuestIds||[]);state.comparisonApplicable=new Set(comparisonProfile?.allApplicableQuestIds||[]);state.applicable=new Set([...state.primaryApplicable,...state.comparisonApplicable]);
  state.repeatableGroups=state.compareMode?[]:profile?.repeatableQuestGroups||[];state.repeatableEndById=new Map();
...
function applyConfig(config){
  ...
  state.compareMode=Boolean(config.compareMode&&state.comparisonProfile);
  ...
  state.prerequisiteIds=new Set(config.prerequisiteIds||[]);state.successorIds=new Set(config.successorIds||[]);state.highlightedEdges=new Set(config.highlightedEdgeIndexes||[]);state.comparisonById=new Map((config.comparisons||[]).map(item=>[item.questId,item]));
}
...
function composeRepeatableLayout(base){
  const groups=state.repeatableGroups.map(group=>({kind:group.kind,scav:Boolean(group.scav),endTime:group.endTime,entries:(group.quests||[]).filter(entry=>state.visible.has(entry.node.id))})).filter(group=>group.entries.length);
...
function effectiveComparisonQuestState(id){return state.comparisonStateById.get(id);}
function applicableDisplay(id,questState,applicable){return applicable?(questState?.displayState||'Locked'):'NotApplicable';}
function combinedDisplayState(id){const values=[];if(state.primaryApplicable.has(id))values.push(effectiveQuestState(id)?.displayState||'Locked');if(state.comparisonApplicable.has(id))values.push(effectiveComparisonQuestState(id)?.displayState||'Locked');if(values.length&&values.every(value=>value==='Completed'))return'Completed';if(values.some(value=>['Available','InProgress','ReadyToFinish'].includes(value)))return'InProgress';return'Locked';}
function isFutureForEveryApplicableProfile(id){const values=[];if(state.primaryApplicable.has(id))values.push(effectiveQuestState(id));if(state.comparisonApplicable.has(id))values.push(effectiveComparisonQuestState(id));return Boolean(values.length)&&values.every(isFutureQuest);}
...
function drawNode(node,questState,p,dimmed,dimAlpha=.20){
  ...
  const prerequisite=state.prerequisiteIds.has(node.id),successor=state.successorIds.has(node.id),hovered=node.id===state.hoverId,hoverPredecessor=state.hoverPredecessorIds.has(node.id),hoverSuccessor=state.hoverSuccessorIds.has(node.id),emphasized=selected||prerequisite||successor||hovered||hoverPredecessor||hoverSuccessor,nodeAlpha=dimmed?dimAlpha:future&&!emphasized?.56:1,body={x:p.x+6,y:p.y,w:p.w-6,h:p.h},hasRoute=state.collectorPath.has(node.id)||state.lightkeeperPath.has(node.id);
  ...
  if(node.scavRepeatable)drawScavBadge(p,completed);else if(node.eventSeason)badge(p.x+p.w-(completed?36:13),p.y+12,t('badge.event'),state.theme.badgeEvent);else if(node.exclusionRules?.length)badge(p.x+p.w-(completed?36:13),p.y+12,t('badge.branch'),state.theme.badgeBranch);if(isTerminalQuest(node.id))drawTerminalMarker(p);if(completed)drawCompletedMarker(p);drawNodeOutline(p,selected,prerequisite,successor,hovered,hoverPredecessor,hoverSuccessor);if(hasDirectPrerequisite(node.id))drawCenterNotch(p);ctx.restore();
}
function drawCompareNode(node,primaryState,comparisonState,p,dimmed,matchingDimmed){
  const comparison=state.comparisonById.get(node.id)||{categories:[],hasDifferences:false};
  if(!comparison.hasDifferences){const shared=state.primaryApplicable.has(node.id)?primaryState:comparisonState;drawNode(node,shared,p,dimmed||matchingDimmed,dimmed?.20:.35);return;}
  const ctx=state.ctx,primaryDisplay=applicableDisplay(node.id,primaryState,state.primaryApplicable.has(node.id)),comparisonDisplay=applicableDisplay(node.id,comparisonState,state.comparisonApplicable.has(node.id)),primaryColor=state.theme.states[primaryDisplay]||state.theme.states.Locked,comparisonColor=state.theme.states[comparisonDisplay]||state.theme.states.Locked,selected=node.id===state.selectedId,future=isFutureForEveryApplicableProfile(node.id);
  const prerequisite=state.prerequisiteIds.has(node.id),successor=state.successorIds.has(node.id),hovered=node.id===state.hoverId,hoverPredecessor=state.hoverPredecessorIds.has(node.id),hoverSuccessor=state.hoverSuccessorIds.has(node.id),emphasized=selected||prerequisite||successor||hovered||hoverPredecessor||hoverSuccessor,nodeAlpha=dimmed?.20:future&&!emphasized?.72:1,body={x:p.x+7,y:p.y+1,w:p.w-8,h:p.h-2},hasRoute=state.collectorPath.has(node.id)||state.lightkeeperPath.has(node.id);
  ctx.save();ctx.globalAlpha=nodeAlpha;roundRect(p.x,p.y,p.w,p.h,8);ctx.fillStyle=state.theme.compareDifference;ctx.fill();roundRect(body.x,body.y,body.w,body.h,7);ctx.fillStyle=state.theme.cardBody;ctx.fill();ctx.save();roundRect(body.x,body.y,body.w,body.h,7);ctx.clip();ctx.globalAlpha=1;const hasBanner=drawCardBanner(node,body);if(hasBanner){ctx.fillStyle=state.theme.bannerShade;ctx.fillRect(body.x,body.y,body.w,body.h);}ctx.restore();drawRouteStrip(node.id,body);
  if(state.view.scale<OVERVIEW_SCALE){const barX=body.x+10,barW=Math.max(18,body.w-20);ctx.fillStyle=primaryColor;roundRect(barX,p.y+18,barW,14,3);ctx.fill();ctx.fillStyle=comparisonColor;roundRect(barX,p.y+52,barW,14,3);ctx.fill();if(state.view.scale>=.20)drawComparisonReasonBadge(p,comparison.categories,true);if(node.eventSeason){ctx.fillStyle=state.theme.eventMarker;ctx.fillRect(p.x+p.w-12,p.y+7,5,5);}if(isTerminalQuest(node.id))drawTerminalMarker(p);drawComparisonOutline(p);drawNodeOutline(p,selected,prerequisite,successor,hovered,hoverPredecessor,hoverSuccessor);if(hasDirectPrerequisite(node.id))drawCenterNotch(p);ctx.restore();return;}
  const iconSize=51,iconX=body.x+9,iconY=p.y+(hasRoute?8.5:(p.h-iconSize)/2),traderImage=getImage(node.traderImageUrl);if(traderImage?.complete&&traderImage.naturalWidth){ctx.save();roundRect(iconX,iconY,iconSize,iconSize,5);ctx.clip();drawImageCover(traderImage,iconX,iconY,iconSize,iconSize);ctx.restore();}else{ctx.fillStyle=state.theme.portraitBackground;roundRect(iconX,iconY,iconSize,iconSize,5);ctx.fill();ctx.fillStyle=state.theme.portraitText;ctx.font='700 16px system-ui';ctx.textAlign='center';ctx.fillText(initials(node.traderName),iconX+iconSize/2,iconY+32);}ctx.strokeStyle=state.theme.portraitBorder;ctx.lineWidth=1/state.view.scale;roundRect(iconX,iconY,iconSize,iconSize,5);ctx.stroke();
  const textX=iconX+iconSize+9,textWidth=p.x+p.w-13-textX,badgeWidth=drawComparisonReasonBadge(p,comparison.categories,false);ctx.save();ctx.shadowColor=state.theme.textShadow;ctx.shadowBlur=3;ctx.shadowOffsetX=1;ctx.shadowOffsetY=1;ctx.textAlign='left';ctx.fillStyle=state.theme.title;ctx.font='700 13px system-ui';ctx.fillText(fitText(node.name,Math.max(30,textWidth-badgeWidth-5)),textX,iconY+12);drawCompareStatusLine('A',primaryDisplay,primaryState,textX,iconY+31,textWidth,primaryColor);drawCompareStatusLine('B',comparisonDisplay,comparisonState,textX,iconY+48,textWidth,comparisonColor);ctx.restore();
  if(node.eventSeason){ctx.fillStyle=state.theme.eventMarker;ctx.fillRect(p.x+p.w-10,p.y+p.h-10,4,4);}if(isTerminalQuest(node.id))drawTerminalMarker(p);drawComparisonOutline(p);drawNodeOutline(p,selected,prerequisite,successor,hovered,hoverPredecessor,hoverSuccessor);if(hasDirectPrerequisite(node.id))drawCenterNotch(p);ctx.restore();
}
function drawCompareStatusLine(label,display,questState,x,y,width,color){const ctx=state.ctx;ctx.fillStyle=color;roundRect(x,y-8,13,11,2);ctx.fill();ctx.fillStyle=state.theme.title;ctx.font='800 7px system-ui';ctx.textAlign='center';ctx.fillText(label,x+6.5,y);ctx.textAlign='left';ctx.font='700 8px system-ui';const progress=display==='InProgress'&&questState?.progressPercent!=null?` · ~${number(questState.progressPercent)}%`:'';ctx.fillText(fitText(`${stateLabel(display).toLocaleUpperCase(state.browserLocale)}${progress}`,width-18),x+18,y);}
function comparisonReason(categories){const label=t(`compare.category.${categories[0]}`);return categories.length>1?`${label} +${categories.length-1}`:label;}
function drawComparisonReasonBadge(p,categories,overview){const ctx=state.ctx,label=comparisonReason(categories);ctx.save();ctx.font=`800 ${overview?11:8}px system-ui`;const maxWidth=overview?p.w-40:82,width=Math.min(maxWidth,ctx.measureText(label).width+12),height=overview?17:14,x=p.x+p.w-width-8,y=p.y+(overview?(p.h-height)/2:6);ctx.fillStyle=state.theme.compareDifference;roundRect(x,y,width,height,3);ctx.fill();ctx.fillStyle=state.theme.compareDifferenceText;ctx.textAlign='center';ctx.fillText(fitText(label,width-8),x+width/2,y+(overview?12:10));ctx.restore();return width;}
function drawComparisonOutline(p){const ctx=state.ctx;ctx.save();ctx.strokeStyle=state.theme.compareDifference;ctx.lineWidth=2.4/state.view.scale;roundRect(p.x,p.y,p.w,p.h,8);ctx.stroke();ctx.restore();}
...
function usesActualMaps(location){return Boolean(location?.any)||location?.id?.toLocaleLowerCase()==='marathon';}
function displayMapReferences(node){if(usesActualMaps(node.location)&&node.actualMapsComplete&&node.actualMaps?.length)return node.actualMaps;if(node.location&&!node.location.any)return[node.location];return[];}
function drawCardBanner(node,body){const quest=getImage(node.imageUrl),mapReferences=displayMapReferences(node),mapKeys=mapReferences.map(map=>`${map.id}:${map.bannerImageUrl||''}`),maps=mapReferences.map(map=>getImage(map.bannerImageUrl)),questReady=Boolean(quest?.complete&&quest.naturalWidth),mapReady=maps.some(map=>map?.complete&&map.naturalWidth);if(!questReady&&!mapReady)return false;if(questReady)drawImageCover(quest,body.x,body.y,body.w,body.h);if(mapReferences.length){const layer=getMapLayer(mapKeys,maps,body.w,body.h,questReady);state.ctx.drawImage(layer,body.x,body.y,body.w,body.h);}return true;}
function getMapLayer(mapKeys,maps,width,height,preserveQuestArtwork){const key=`${mapKeys.join(',')}|${width}x${height}|${preserveQuestArtwork?'right':'full'}`;if(state.mapFadeCache.has(key))return state.mapFadeCache.get(key);const layer=document.createElement('canvas');layer.width=Math.ceil(width);layer.height=Math.ceil(height);const ctx=layer.getContext('2d'),mapLeft=preserveQuestArtwork?layer.width*.6:0,mapWidth=layer.width-mapLeft;drawAngledMapSlices(ctx,maps,mapLeft,mapWidth,layer.height);state.mapFadeCache.set(key,layer);return layer;}
function drawAngledMapSlices(ctx,maps,mapLeft,mapWidth,height){const sliceWidth=mapWidth/maps.length,skew=height*.18,outerRight=mapLeft+mapWidth;for(let index=0;index<maps.length;index++){const left=mapLeft+index*sliceWidth,right=mapLeft+(index+1)*sliceWidth,map=maps[index],flushLeft=index===0&&mapLeft===0,leftTop=flushLeft?0:left+skew,leftBottom=flushLeft?0:left-skew,rightTop=index===maps.length-1?outerRight:right+skew,rightBottom=index===maps.length-1?outerRight:right-skew,drawLeft=Math.min(leftTop,leftBottom),drawRight=Math.max(rightTop,rightBottom);ctx.save();ctx.beginPath();ctx.moveTo(leftTop,0);ctx.lineTo(rightTop,0);ctx.lineTo(rightBottom,height);ctx.lineTo(leftBottom,height);ctx.closePath();ctx.clip();if(map?.complete&&map.naturalWidth)drawImageCoverTo(ctx,map,drawLeft,0,drawRight-drawLeft,height);ctx.restore();if(!flushLeft){ctx.strokeStyle=state.theme.mapDivider;ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(leftTop,0);ctx.lineTo(leftBottom,height);ctx.stroke();}}}
...
function drawScavBadge(p,completed){const ctx=state.ctx,label=t('repeatable.scav').toLocaleUpperCase(state.browserLocale);ctx.save();ctx.font='800 8px system-ui';const width=Math.max(34,ctx.measureText(label).width+12),x=p.x+p.w-width-(completed?31:7),y=p.y+6;ctx.fillStyle=state.theme.badgeScav;roundRect(x,y,width,15,3);ctx.fill();ctx.fillStyle=state.theme.badgeScavText;ctx.textAlign='center';ctx.fillText(label,x+width/2,y+10.5);ctx.restore();}
...
function updateMetrics(){if(!state.metrics||!state.topology||!state.profile||!state.activeLayout)return;const differences=[...state.comparisonById.values()].filter(item=>item.hasDifferences).length;state.metrics.textContent=t('metrics.summary',{visible:state.visible.size,applicable:state.applicable.size,edges:state.visibleEdgeCount,layout:state.activeLayout.layoutMs.toFixed(1),overlay:'0.0',render:state.lastRenderMs.toFixed(1)})+(state.compareMode?` · ${differences} ${t('compare.differencesShort')}`:'')+(state.view.scale<OVERVIEW_SCALE?t('metrics.overview'):'');}

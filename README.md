...
SPT-QuestMap 2.1.2 is a **full in-game questing replacement** for **SPT 4.1.6+**. It replaces both the global **Character → Tasks** screen and every trader's **Tasks** tab with a complete quest workspace for browsing, filtering, accepting, progressing, tracking, handing in, and planning quests without leaving EFT.
...
This release targets **SPT >=4.1.6 <4.2.0** with EFT **0.16.9.40743**. The initial build and metadata validation target is SPT 4.1.6. Later 4.1 patches are admitted only when the known EFT assembly fingerprint and all required native contracts match. SPT 4.0 support remains in the 2.0 releases. Live EFT and Fika acceptance for this port is pending; compilation and automated tests do not establish live compatibility.

The native client validates the installed identity and 71 required UI/action/event contracts before activating. A mismatch safe-disables the custom screens and leaves the vanilla interfaces available instead of guessing at a changed ABI.
...
The combined archive is the recommended installation. For a server/browser-only setup, install only `SPT_Runtime/user/mods/SPT-QuestMap/`; the BepInEx client is not required. A client-only installation is not supported: `BepInEx/plugins/SPTQuestMap/` requires the matching server component to be installed and running.

1. Back up the profiles you care about.
2. Stop the SPT server and close EFT.
3. Download the combined release archive and extract it into the Tarkov installation directory containing `SPT_Runtime`, `BepInEx`, and `EscapeFromTarkov.exe`. The archive already includes both complete install-relative paths.
4. Allow the archive's `SPT_Runtime` and `BepInEx` folders to merge with the existing ones and overwrite an older QuestMap release. Confirm the main runtime files exist:

   ```text
   <Tarkov root>/SPT_Runtime/user/mods/SPT-QuestMap/SPTQuestMap.dll
   <Tarkov root>/SPT_Runtime/user/mods/SPT-QuestMap/SPTQuestMap.Core.dll
   <Tarkov root>/SPT_Runtime/user/mods/SPT-QuestMap/Data/metainfo.json
   <Tarkov root>/SPT_Runtime/user/mods/SPT-QuestMap/Data/triggerIds.json
...
Task skipping is implemented directly against the guarded EFT `0.16.9.40743` quest-condition controller; SPT-Skipper is not required.
...
```text
SPT_Runtime/user/mods/SPT-QuestMap/summaries/
```
...
- the .NET 10 SDK;
- a matching SPT 4.1.6 installation containing `SPT_Runtime`, `BepInEx`, `EscapeFromTarkov.exe`, and the target client assemblies; and
- the matching SPT 4.1.6 source under `reference/spt-4.1.6-sources/` when auditing or changing SPT integration behavior.

SPT, EFT, Unity, and BepInEx assemblies remain external read-only references and are not committed or redistributed.

Pass the Tarkov installation directory—the parent of `SPT_Runtime`, not the `SPT_Runtime` directory itself—to the build script:
...
The combined build validates SPT 4.1.6, builds the server and client, runs the shared-core and server/browser tests, and stages clean outputs under `dist/client/` and `dist/server/`.
...
The deployment helper limits writes to the two QuestMap install directories. Client deployment does not inspect or stop EFT. If server DLLs change, the helper requires and invokes your restart command through `-RestartCommand` or the ignored `scripts/restart-command.local.txt`. No restart command is inferred.
...
pwsh -NoProfile -ExecutionPolicy Bypass -File .\scripts\package-release.ps1 `
  -Target Both -Configuration Release -Version 2.1.2
```
...
- `SPTQuestMap`: the .NET 10 SPT server mod, Blazor page, profile projection, localization, and sanitized client feeds;
